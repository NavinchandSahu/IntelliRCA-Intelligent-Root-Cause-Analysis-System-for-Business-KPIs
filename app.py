...
def run_analysis(df):
    ...
    if "Product" in df.columns:
        st.subheader("📊 Product-wise Profit")
    ...
    # FACTOR IMPACT BAR
    # =========================
    st.subheader("📊 Factors Impacting Revenue")
    ...
    if {"Region", "Product", "Sales"}.issubset(df.columns):
        st.subheader("📍 Product Popularity by Region")
    ...
    if loss_ratio > 0.4:
        st.warning("⚠️ High loss-making transactions")
    elif loss_ratio > 0.2:
        st.warning("⚠️ Moderate losses detected")
    else:
    ...
    if "Product" in df.columns:
        ...
        if popularity_metric:
            ...
            st.success(
                ...
                • {popularity_metric}: {popularity[most_popular]:,.0f}  
                • Revenue: ₹ {revenue_by_product[most_popular]:,.0f}  
                • Profit: ₹ {profit_by_product[most_popular]:,.0f}
                """
            )

            st.warning(
                f"""
                📉 **Least Popular Product:** {least_popular}  
                • {popularity_metric}: {popularity[least_popular]:,.0f}  
                • Revenue: ₹ {revenue_by_product[least_popular]:,.0f}  
                • Profit: ₹ {profit_by_product[least_popular]:,.0f}
                """
            )
    ...
    if loss_df.empty:
        st.success("🎉 No major losses detected")
    else:
        # 1️⃣ Product causing maximum loss
        if "Product" in df.columns:
            loss_product = loss_df.groupby("Product")["Profit"].sum().idxmin()
            st.error(f"❌ Highest Loss Product: {loss_product}")

        # 2️⃣ Region causing maximum loss
        if "Region" in df.columns:
            loss_region = loss_df.groupby("Region")["Profit"].sum().idxmin()
            st.error(f"📍 Highest Loss Region: {loss_region}")

        # 3️⃣ Date with maximum loss
        if "Date" in df.columns:
            loss_date = (
        ...
        # 4️⃣ Cost-heavy products (Cost >> Revenue)
        cost_imbalance = (
        ...
        if not cost_imbalance.empty:
            ...
            st.warning(
                f"💸 Cost Inefficiency: {worst_cost_product} "
                f"(Cost is {cost_imbalance.iloc[0]['Cost_Ratio']:.2f}x Revenue)"
            )

        # 5️⃣ High sales but low profit products
        if {"Sales", "Product"}.issubset(df.columns):
            ...
            if not risky_product.empty:
                st.warning(
                    f"⚠️ High Sales but Loss-Making Product: {risky_product.index[0]}"
                )

        # 6️⃣ Region with highest loss ratio
        if "Region" in df.columns:
            region_loss_ratio = (
            ...
            st.warning(
                f"📊 Highest Loss Ratio Region: {worst_loss_region} "
                f"({region_loss_ratio.iloc[0]*100:.1f}% loss transactions)"
            )

        # 7️⃣ Anomaly-based RCA
        if "Anomaly" in df.columns and "Product" in df.columns:
            ...
            if not anomaly_products.empty:
                st.warning(
                    f"🚨 Most Anomaly-Prone Product: {anomaly_products.index[0]}"
                )

        # 8️⃣ Low margin products
        margin = (
        ...
        if not margin.empty:
            st.warning(
                f"📉 Lowest Margin Product: {margin.index[0]} "
                f"({margin.iloc[0]['Margin']*100:.1f}% margin)"
            )

        # 9️⃣ Region–Product loss combination
        if {"Region", "Product"}.issubset(df.columns):
            ...
            if not combo_loss.empty:
                ...
                st.error(
                    f"🔗 Loss Hotspot: {product} in {region}"
                )
        ...
        if "Date" in df.columns:
            ...
            if not spike.empty:
                st.warning(
                    f"⚡ Sudden Cost Spike Detected on: {spike.index[0].date()}"
                )

        # 1️⃣1️⃣ Revenue drop despite stable sales
        if {"Sales", "Revenue"}.issubset(df.columns):
            corr_sr = df["Sales"].corr(df["Revenue"])
            if corr_sr < 0.3:
                st.warning(
                    "📉 Revenue is weakly correlated with Sales — possible pricing or discount issue"
                )

        # 1️⃣2️⃣ Seasonal loss pattern
        if "Date" in df.columns:
            ...
            if not monthly_loss.empty:
                st.warning(
                    f"📆 Worst Performing Month: {monthly_loss.index[0]}"
                )
...
# =========================
if menu == "🏠 Home":
    st.title("📊 Intelligent Root Cause Analysis System")
    st.markdown("""
    Welcome to the **Intelligent RCA System for Business KPIs**.

    🔍 Detect profit & loss anomalies  
